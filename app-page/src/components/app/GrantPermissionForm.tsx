...
const grantSchema = z.object({
  permission_id: z.string().min(1, 'Select a permission').max(255),
});
...
export default function GrantPermissionForm({ uid, allPermissions, onGranted, lang = 'en' }: GrantPermissionFormProps) {
  ...
  const grantPermission = async (e: React.FormEvent) => {
    e.preventDefault();
    const parsed = grantSchema.safeParse({ permission_id: grantId });
    if (!parsed.success) {
    ...
    const res = await fetch(`/api/user/${uid}/permissions/`, {
      ...
      body: JSON.stringify({ permission_id: grantId, granted: true }),
    });
    if (res.ok) {
