...
export default function UsersManager({
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; username: string; isDeleted: boolean } | null>(null);
  ...
  const handleDelete = (uid: string, username: string, isDeleted: boolean) => {
    setDeleteTarget({ id: uid, username, isDeleted });
  };

  const confirmSingleDelete = async () => {
    if (!deleteTarget) return;
    const { id: uid, isDeleted } = deleteTarget;
    setDeleteTarget(null);
    const endpoint = isDeleted ? `/api/user/${uid}/forever` : `/api/user/${uid}`;
          ...
          actions={(user: any) => (
            <div className="flex items-center justify-end gap-2">
              <Button
                ...
                size="icon"
                disabled={!canManage}
                onClick={() => handleDelete(user.id, user.username, !!user.deleted_at)}
          ...
          toolbar={
            <>
              ...
              {selectedUids.size > 0 && canManage && (
                <div className="border-border flex items-center gap-3 border-t px-6 py-3">
      ...
      <Dialog open={deleteTarget !== null} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Delete user &ldquo;{deleteTarget?.username}&rdquo;? {deleteTarget?.isDeleted ? 'This cannot be undone.' : 'This can be undone.'}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteTarget(null)}>{t('common.cancel')}</Button>
            <Button variant="destructive" onClick={confirmSingleDelete}>{t('common.delete')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
