...
import type { UpdatePasswordDto } from 'diva-types/user/dtos';
...
export async function PATCH({ params, request, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const body: UpdatePasswordDto = await request.json();

    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/password`, {
      method: 'PATCH',
      headers: {
