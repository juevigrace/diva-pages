...
import type { UpdateEmailDto } from 'diva-types/user/dtos';
...
export async function PATCH({ params, request, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const body: UpdateEmailDto = await request.json();

    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/email`, {
      method: 'PATCH',
      headers: {
