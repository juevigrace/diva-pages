...
import type { UpdateUserPermissionDto } from 'diva-types/user/dtos';
...
export async function GET({ params, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/permissions/${params.pid}`, {
...
export async function PUT({ params, request, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const body: UpdateUserPermissionDto = await request.json();
    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/permissions/${params.pid}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
...
export async function DELETE({ params, callAction }: APIContext): Promise<Response> {
  try {
    ...
    if (!res.ok) { const json: APIResponse<unknown> = await res.json(); return new Response(JSON.stringify(json), { status: res.status, headers: { 'Content-Type': 'application/json' } }); }
