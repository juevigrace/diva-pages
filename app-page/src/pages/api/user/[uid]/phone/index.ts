...
import type { UpdatePhoneNumberDto } from 'diva-types/user/dtos';
...
export async function PATCH({ params, request, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const body: UpdatePhoneNumberDto = await request.json();

    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/phone`, {
      method: 'PATCH',
      headers: {
