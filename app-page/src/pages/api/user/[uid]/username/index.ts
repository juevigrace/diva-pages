...
import type { UpdateUsernameDto } from 'diva-types/user/dtos';
...
export async function PATCH({ params, request, callAction }: APIContext): Promise<Response> {
  try {
    ...
    const body: UpdateUsernameDto = await request.json();

    const res = await fetch(`${API_BASE_URL}/api/user/${params.uid}/username`, {
      method: 'PATCH',
      headers: {
