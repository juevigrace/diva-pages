...
export async function GET({ params, callAction }: APIContext): Promise<Response> {
  try {
    ...
    return dataResponse<UserResponse>(await apiGet(`/api/user/${params.uid}`, session.access_token));
  } catch (e) {
    return apiError(e);
  }
}
...
export async function DELETE({ params, callAction }: APIContext): Promise<Response> {
  try {
    ...
    if (!result.ok) return result.error;
    const { session } = result;
    return nullResponse(await apiDelete(`/api/user/${params.uid}`, session.access_token));
  } catch (e) {
    return apiError(e);
  }
}
